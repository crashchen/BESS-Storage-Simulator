...
Batch 7 is prepared on `codex/audit-batch-7-scene-clarity` based on deployed `9b81a7b`; it has not been merged or deployed. It gives scene labels a fixed readable screen size, explains the color and direction of energy flows, and shows visible 3D/chart loading states. User-arranged CC review caught a label pointer-event regression and three smaller visual issues; the follow-up now lets clicks reach the Canvas, keeps labels below the HUD, separates labels at 421–455px widths, and restores the red Grid overload label. Node 24.21.0 lint, **223 tests across 21 files**, and the Pages-path build pass. Local browser checks cover mobile/desktop viewports, clicking the PCS label to select its equipment, red overload styling at the evening peak, and successful 8-second-delayed 3D/chart loading without console errors. The follow-up has local validation but no second CC review. See the [batch 7 review guide](docs/audits/2026-09-22/batch-7.md). No simulation or settlement changes.
