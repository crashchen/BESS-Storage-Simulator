...
**实施状态（2026-09-08）**：用户随后批准的第一批及 CC 反馈的五处收尾已在本地完成，lint/build 和 146 个测试（15 文件）通过，README/CLAUDE 与五篇 vault 笔记已同步；交付分支为 `codex/audit-batch-1-demo-ux`，本次交付范围为本地提交，尚未发布。具体变更、验证边界与剩余项见 [第一批交付记录](batch-1.md)。以下保留原审计证据，不将已修复项描述为从未存在。
...
| 顺序 | 交付范围 | 粗估 | 验收条件 |
|---|---|---|---|
| 1：演示体验与状态保持（已本地完成） | 太阳面向；Retry保留状态；抽屉resize与信息卡遮挡；电价时间刻度、输入回显；已确认vault事实纠正 | 原估0.5–1天 | 验收结果见 [第一批交付记录](batch-1.md)；本地分支交付，未部署 |
| 2：模型口径与解释 | yield从modelled改为未入模的reference；overload价值加明确限定或拆项；说明反映实际动作/限制原因 | 0.5–1.5天，拆累计字段需额外回归 | 显示含义与公式一致；不能把恢复供电部分声称为进口抵扣；0MW/满电/reserve/暂停解释正确 |
| 3：数值收敛（P2） | SoC及策略事件拆步；压力案例和正常工况回归 | 0.5–1天 | 相同初态及结束时刻，分步结果收敛；逐来源能量/成本可核对；保留午夜和默认08:00整日对照 |
