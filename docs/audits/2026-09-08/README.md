...
| 8：累计价值拆分（工作树待复审） | BESS出口、实际避免进口和恢复未供负荷的假设价值逐子步累计；旧总额不改；两张主卡明确为演示价值 | 本轮完成 | 228/21、lint/Pages build；350/300 MW过载、负电价、正常负荷/出口、跨步累计及390px浏览器明细见[第八批记录](../2026-09-23/batch-8.md)；未合并/部署 |
| 持续：housekeeping及剩余体验 | 各批次同步仓库/vault；第五、六批已合并发布；第七批PR #10已合并发布；横屏图例、Grid标签点击及NumericField Reset列为独立待办；累计价值拆分与多日AUTO策略分开决策 | 随批次同步 | 文档记录核对日期、commit、检查与部署run；历史drift清单不等于当前全部未修；未完成项保留清单 |
