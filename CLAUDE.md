...
- **Display-only flow decomposition** in `src/utils/energyFlowTelemetry.ts` turns `GridState` into the 6 visible energy-flow legs (`solarToBess` / `solarToLoad` / `solarToExport` / `bessToLoad` / `bessToExport` / `gridToBess` / `gridToSite`). The 3D scene and any flow UI consume from here, not directly from raw state. `src/utils/sceneAssetInfo.ts` supplies the structured payload for the hover/click info card on BESS / PCS-MV / Grid Node.
