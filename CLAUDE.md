...
### Seventh-batch scene clarity (2026-09-22, local review worktree)

Batch 7 is tracked in PR #10 from main `9b81a7b`. User-arranged CC reviewed the initial worktree and found that Drei `Html` ignores its `pointerEvents` prop outside transform mode; use `style={{ pointerEvents: 'none' }}` so labels pass clicks to Canvas. CC independently rechecked that fix plus HUD stacking, portrait label spacing and the red Grid overload label. A further short-landscape media rule lifts PCS at ≤430px viewport height. Local browser checks cover clicking the PCS label, default-overview label spacing and the overloaded Grid label. See `docs/audits/2026-09-22/batch-7.md` for exact evidence and remaining follow-ups. Node 24.21.0 lint, 223 tests / 21 files and the Pages-path build pass; simulation strategy, state and settlement are unchanged. Do not infer a production release from branch checks alone.

PR #10 was merged as `2989452`; main CI 35789638276 and Pages 35789638697 both succeeded on that commit. A live 640×360 browser check confirmed the label separation and showed the already-recorded Solar/legend overlap. The Three vendor chunk warning remains at 724.92 kB. This release does not resolve cumulative value decomposition, multi-day AUTO strategy, Grid-label click behavior or the older `NumericField` Reset draft bug.
