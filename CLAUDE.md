...
`codex/audit-batch-7-scene-clarity` starts at deployed main `9b81a7b` and has not been merged or deployed. User-arranged CC reviewed the initial worktree and found that Drei `Html` ignores its `pointerEvents` prop outside transform mode; use `style={{ pointerEvents: 'none' }}` so labels pass clicks to Canvas. The follow-up also puts HUD above labels, lifts PCS through 480px widths, and restores red Grid overload text. Browser checks cover clicking the PCS label, default-overview label spacing and the overloaded Grid label; the follow-up has not had a second CC review. See `docs/audits/2026-09-22/batch-7.md` for exact evidence. Node 24.21.0 lint, 223 tests / 21 files and the Pages-path build pass; simulation strategy, state and settlement are unchanged. Do not describe these branch checks as a new production release.

`SceneLabel` anchors equipment names to their Three positions through Drei `Html`, but keeps CSS font size stable across full-site camera fits. It is pointer-transparent and below the HUD/drawers. The less important Local Load tag is hidden at ≤420px; the flow legend still names local load. The PCS tag has a narrow-screen upward offset and the Grid tag a small lateral offset so the default 320px and 390px full-site views do not overlap. Arbitrary manual camera poses are not collision-managed. The original metre-sized label config fields were removed; do not reintroduce world-space font scaling for these tags.
