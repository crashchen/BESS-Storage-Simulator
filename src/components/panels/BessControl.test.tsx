...
describe('BessDispatchControl active-power copy', () => {
    it('shows AUTO dispatch and the configured evening entry target', () => {
        ...
        expect(screen.getByTestId('auto-evening-hold-threshold')).toHaveTextContent('peak tariff above about €178.78/MWh');
        expect(screen.getByTestId('auto-evening-hold-threshold')).toHaveTextContent('load, PV, PCC and transfer limits');
    });
    ...
    it('updates the estimated crossover with tariffs and explains when it is unreachable', () => {
        ...
        expect(screen.getByText('Night charge target 40%')).toBeInTheDocument();
        expect(screen.getByTestId('auto-evening-hold-threshold')).toHaveTextContent('No editable peak tariff clears this threshold');
    });
    ...
    it('shows paused snapshot power separately from manual intent after a tariff edit', () => {
        ...
        expect(screen.queryByText(/Peak entry target 40%/)).not.toBeInTheDocument();
        expect(screen.queryByTestId('auto-evening-hold-threshold')).not.toBeInTheDocument();
